...
const Header = () => {
  ...
  const getEthBalance = async (address) => {
    try {
      ...
      const balanceEth = web3.utils.fromWei(balanceWei.toString(), 'ether'); // Convert from Wei to Ether
  ...
  useEffect(() => {
    const fetchBalance = async () => {
      if (user == null) return; // check for user existence
      const balance = await getEthBalance(user.wallet_address); // wait for the balance to resolve
      if (balance !== null) {
        setEthBalance(balance); // set balance state with the resolved value
      }
    };
    fetchBalance(); // execute the async function
  }, [user]);
  ...
  // const preBalance = getBalance(user.wallet_address)
